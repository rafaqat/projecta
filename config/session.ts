...
const sessionConfig = defineConfig({
  ...
  cookie: {
    ...
    /**
     * Send cookies only over HTTPS in production. The local and test stacks
     * are served over plaintext HTTP, where a Secure cookie is never returned
     * by the browser; relax it there (same convention as oidc_client.ts).
     */
    secure: app.inProduction && !['local', 'test'].includes(env.get('APP_ENV')),
